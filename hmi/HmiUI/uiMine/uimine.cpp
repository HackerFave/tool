...
#include "ui_uimine.h"
#include <QDebug>
#include "zmqmodule/dataInterface.h"
    ...
    , ui(new Ui::uiMine)
{
    ...
    delayChoice = new DelayChoice;//按钮组界面
    ...
    faultMineChoice = new FaultMineChoice;//故障申报界面
    ...
    ui->editionTable->item(5,1)->setText(QString("V%1.%2.%3").arg(QString::number(DEV_COMPAT_VERSION, 10), 2, QLatin1Char('0'))
                                         .arg(QString::number(DEV_MAIN_VERSION, 10), 2, QLatin1Char('0'))
    ...
    connect(dataInterface::getInstance(),SIGNAL(signalTopicData(const void *,int)),this,SLOT(slotTopicData(const void*,int)));
}
...
void uiMine::on_workSetBtn_clicked()
{
    preWarnCentralWorkStatePage=NULL;
    preWarnControlStackedWidget=NULL;
...
void uiMine::on_otherSetBtn_clicked()
{
    ui->controlStackedWidget->setCurrentWidget(ui->controlOtherSetPage);//去往其他设置页面
}

void uiMine::on_loginOutBackBtn_clicked()
{
...
void uiMine::on_loginOutOkBtn_clicked()
{
    ui->controlStackedWidget->setCurrentWidget(ui->controlMainPage);//返回首界面
    ui->centralStackedWidget->setCurrentWidget(ui->centralMapViewPage);//返回主显示页面
}

void uiMine::on_outWorkBackBtn_clicked()
{
    ui->centralStackedWidget->setCurrentWidget(ui->centralWorkStatePage);
    ui->controlStackedWidget->setCurrentWidget(ui->controlWorkSetPage);//返回首界面
}

void uiMine::on_stateSetBackBtn_clicked()
{
    ui->controlStackedWidget->setCurrentWidget(ui->controlMainPage);//返回首界面
}

//状态设置确认按下
void uiMine::on_workSetOkBtn_clicked()
{
...
//状态设置返回按下
void uiMine::on_workSetBackBtn_clicked()
{
    if(preWarnControlStackedWidget!=NULL)
        return;
    ui->centralStackedWidget->setCurrentWidget(ui->centralMapViewPage);
...
//载货状态返回
void uiMine::on_loadStateBackBtn_clicked()
{
    ui->centralStackedWidget->setCurrentWidget(ui->centralWorkStatePage);
...
//加油返回
void uiMine::on_addOilBackBtn_clicked()
{
    ui->centralStackedWidget->setCurrentWidget(ui->centralWorkStatePage);
    ui->controlStackedWidget->setCurrentWidget(ui->controlWorkSetPage);//返回状态设置页面
}
...
//加油确定
void uiMine::on_addOilOkBtn_clicked()
{
    //发送加油信号
    ui->controlStackedWidget->setCurrentWidget(ui->controlMainPage);//返回主界面
}

void uiMine::on_outWorkOkBtn_clicked()
{
    //发送退出排队信号
...
void uiMine::on_faultBtn_clicked()
{
...
//延时10分钟按键按下
void uiMine::on_delay10Btn_clicked()
{
    //发送延时10分钟信号
...
//延时30分钟按键按下
void uiMine::on_delay30Btn_clicked()
{
    //发送延时30分钟信号
    if(delayChoice->buttonGroup->checkedButton())
    {
...
//延时60分钟按键按下
void uiMine::on_delay60Btn_clicked()
{
    //发送延时60分钟信号
    if(delayChoice->buttonGroup->checkedButton())
    {
...
//延时页面返回
void uiMine::on_delayBackBtn_clicked()
{
    ui->controlStackedWidget->setCurrentWidget(ui->controlStateSetPage);
    ui->centralStackedWidget->setCurrentWidget(ui->centralMapViewPage);
}

void uiMine::on_voiceAdjustBtn_clicked()
{
    ui->controlStackedWidget->setCurrentWidget(ui->controlVoicePage);
}

void uiMine::on_brightAdjustBtn_clicked()
{
    commonUIFunction::instance()->on_brightAdjustBtn_clicked(ui->controlStackedWidget,
                                                             ui->controlBrightPage);
}

void uiMine::on_editionBtn_clicked()
{
    commonUIFunction::instance()->h2gSendHmiVerCMD();
    ui->controlStackedWidget->setCurrentWidget(ui->controlEditionPage);
}

void uiMine::on_otherSetBackBtn_clicked()
{
    ui->controlStackedWidget->setCurrentWidget(ui->controlMainPage);//返回首界面
}

void uiMine::on_brightMinusBtn_clicked()
{
    //亮度调低
}

void uiMine::on_brightPlusBtn_clicked()
{
    //亮度调高
}

void uiMine::on_brightBackBtn_clicked()
{
    ui->controlStackedWidget->setCurrentWidget(ui->controlOtherSetPage);
}

void uiMine::on_editionBackBtn_clicked()
{
    ui->controlStackedWidget->setCurrentWidget(ui->controlOtherSetPage);
}

void uiMine::on_voiceBackBtn_clicked()
{
    ui->controlStackedWidget->setCurrentWidget(ui->controlOtherSetPage);
}

void uiMine::on_emptyOkBtn_clicked()
{
    //发送设置空载信号
...
void uiMine::on_emptyBackBtn_clicked()
{
    ui->controlStackedWidget->setCurrentWidget(ui->controlLoadStatePage);
}

void uiMine::on_halfOkBtn_clicked()
{
    //发送设置半载信号
...
void uiMine::on_halfBackBtn_clicked()
{
    ui->controlStackedWidget->setCurrentWidget(ui->controlLoadStatePage);
}

void uiMine::on_fullOkBtn_clicked()
{
    //发送设置满载信号
...
void uiMine::on_fullBackBtn_clicked()
{
    ui->controlStackedWidget->setCurrentWidget(ui->controlLoadStatePage);
}

void uiMine::g2hPosSlot(struct pubVehiclePosST info)  //网关发送hmi位置信息
{
    PosInfo* pos = PosInfo::getHandle();
    pos->setPosInfo(info);
}

void uiMine::g2hReqWorkOnStatuSlot(struct pubVehicleWorkStatuST workStatu) //平台发送网关运营状态变更
{
    //在信息窗口显示运营状态变更情况
    QString str;
...
void uiMine::g2hResWorkOnStatuSlot(struct pubVehicleResWorkStatuST cloudReq) //收到平台运营状态变更确认
{
    commonUIFunction::instance()->g2hResWorkOnStatuSlot(ui->messageBrowser,cloudReq);

}

void uiMine::g2hReqWorkOptSlot(struct pubVehicleReqWorkOptST reqWork) //网关请求现场作业
{
    QString str;
    str.append(QDateTime::currentDateTime().toString("hh:mm:ss :"));
...
void uiMine::g2hReqWorkRealOptSlot(struct pubVehicleReqWorkOptST reqWork) //网关请求现场作业
{
    QString str;
    str.append(QDateTime::currentDateTime().toString("hh:mm:ss :"));
...
void uiMine::g2hResWorkOptSlot(struct pubVehicleResWorkOptST opt) //辅助车辆（网关已收到矿卡现场作业），矿卡（接收到辅助车辆相应作业信息）
{
...
void uiMine::g2hCollWarnningSlot(struct pubVehicleCollWarnningST otherVehicle) //预警-碰撞检测
{
    //预警显示
...
void uiMine::g2hDownMapFileSlot(int flag, QString path) //接收网关下载地图命令
{
...
void uiMine::g2hVersionSlot(struct versionST versiondataSt) //版本号
{
    commonUIFunction::instance()->g2hVersionSlot(ui->editionTable,versiondataSt,&versiondata);
}

void uiMine::g2hDownTaskFileSlot(bool flag, QString path, QStringList taskList) //下载文件
{
...
void uiMine::g2hDownPermissionFileSlot(bool flag, QStringList permissionList,quint32 endPointNum)
{
    commonUIFunction::instance()->g2hDownPermissionFileSlot(flag,  permissionList, endPointNum,
                                                            ui->messageBrowser,
                                                            map);
}
void uiMine::g2hStartUpSlot(struct startUpST opt) //网关发送开机命令
{
    //信息窗口显示，网关开机
    QString str;
...
void uiMine::g2hShutDownAckSlot() //网关关机应答
{
    //信息窗口显示，网管关机
    QString str;
...
void uiMine::g2hDevNameSlot(struct pubVehicleDevNameST imeiName) //网关检权获取名称后发送给hmi
{
    ...
    map->setSelfVehicle(MINEUNLOADER, QString::fromUtf8(imeiName.localname), QSize(width, length));
}

void uiMine::g2hMatAckSlot(short mater) //物料变更
{
    //显示物料变更为何物
    QString str;
...
void uiMine::g2hReqTaskSlot(struct pubVehicleReqTaskST reqTask) //请求任务
{
    commonUIFunction::instance()->g2hReqTaskSlot(ui->messageBrowser,reqTask);
}

void uiMine::g2hResTaskSlot(struct pubVehicleResTaskST resTask) //接收平台响应任务
{    
    commonUIFunction::instance()->g2hResTaskSlot(ui->messageBrowser,ui->titleDestLabel,resTask,taskInfo);
}
void uiMine::g2hResPermissionSlot(struct pubDrivePermissionListST resPermission) //接收平台响应任务
{

}
void uiMine::g2hReqRoadSlot(struct pubVehicleReqRoadST reqRoad) //申请路权
{
    //信息窗口显示申请路权
    QString str;
...
void uiMine::g2hResRoadSlot(struct pubVehicleResRoadST resRoad) //平台回复路权申请
{
    //信息窗口显示收到路权申请
    QString str;
...
void uiMine::g2hFreeRoadSlot(struct pubVehicleReqRoadST freeRoad) //平台下发释放路权
{
    //信息窗口显示收到平台下发释放路权
    QString str;
...
void uiMine::g2hTemporaryTaskSlot(char type) //平台下发临时调度作业
{
    commonUIFunction::instance()->g2hTemporaryTaskSlot(ui->messageBrowser,type);
}

void uiMine::g2hMasterWarnningSlot(struct pubVehicleMasterWarnningST materWarning) //主控发送预警
{

    qDebug()<<"him获取到信息"<<materWarning.speedx<<materWarning.warnningType<<"TC-----test";
    //    PosInfo* pos = PosInfo::getHandle();
...
void uiMine::g2hGatewayOffline(bool status)
{
    commonUIFunction::instance()->g2hGatewayOffline(ui->messageBrowser,status);
}

void uiMine::insertWarnningSlots(int num)
{
    try
    {
        QString str;
...
void uiMine::removeWarnningSlots(int num)
{
    try
    {
        QString str;
...
void uiMine::titleTimerSlot()//界面头信息更新显示
{
    ...
    else if(workStateInfo.cmdType == PUB_WORK_OPT_OUT_TASK)//可驶离，往前走
    {
...
void uiMine::messageEndTimerSlot()
{
    ui->messageBrowser->moveCursor(QTextCursor::Start);
}

//消息往上滑动
void uiMine::on_messageUpBtn_clicked()
{
...
//信息往下滑动
void uiMine::on_messageDownBtn_clicked()
{
    ui->messageBrowser->moveCursor(QTextCursor::Down);
    messageEndTimer->start(2000);
}

void uiMine::blinkTimerSlot()
{
    if(blinkFlag)
    {
        blinkFlag = false;
...
void uiMine::speedBlinkTimerSlot()
{
    if(speedBlinkFlag)
    {
...
void uiMine::aroundVehicleUpdateTimerSlot()
{
    map->updateVehicleList(aroundVehicleList);
    aroundVehicleList.clear();
}

void uiMine::collectionTimerSlot()
{
    PosInfo* pos = PosInfo::getHandle();
    struct latlngheading curPos;
    pos->getCurPos(&curPos);
...
void uiMine::on_collectionBackBtn_clicked()
{
    ui->centralStackedWidget->setCurrentWidget(ui->centralWorkStatePage);
    ui->controlStackedWidget->setCurrentWidget(ui->controlWorkSetPage);
}

void uiMine::on_startCollectionBtn_clicked()
{
    ui->centralStackedWidget->setCurrentWidget(ui->centralMapViewPage);
...
void uiMine::on_updateDataBtn_clicked()
{
    ui->controlStackedWidget->setCurrentWidget(ui->controlCollectionUpdateDataPage);
}

void uiMine::on_leftBtn_clicked()
{
    ui->suspendRestartBtn->setChecked(false);
    ui->controlStackedWidget->setCurrentWidget(ui->controlCollectionStatePage);
}

void uiMine::on_rightBtn_clicked()
{
    ui->suspendRestartBtn->setChecked(false);
    ui->controlStackedWidget->setCurrentWidget(ui->controlCollectionStatePage);
}

void uiMine::on_suspendRestartBtn_clicked()
{
...
void uiMine::on_stopBtn_clicked()
{
    ui->controlStackedWidget->setCurrentWidget(ui->controlCollectionOverPage);
}

void uiMine::on_collectionOverBackBtn_clicked()
{
    ui->controlStackedWidget->setCurrentWidget(ui->controlCollectionStatePage);
}

void uiMine::on_saveBtn_clicked()
{
    //生成边界文件lpx
    collectionTimer->stop();

    generateRoadLpxFile(QString("%1/%2").arg(ParaParsing::instance()->paraSt.pub.mapPath).arg(QDateTime::currentDateTime().toString("MM-dd-hh-mm-ss.lpx")),collectionList);
    ui->controlStackedWidget->setCurrentWidget(ui->controlCollectionUpdateDataPage);

    collectionList.clear();
}

void uiMine::on_deleteBtn_clicked()
{
    collectionTimer->stop();
    collectionList.clear();
...
void uiMine::on_updateDataOkBtn_clicked()
{
    //提交边界文件
    ui->controlStackedWidget->setCurrentWidget(ui->controlMainPage);
}

void uiMine::on_updateDataBackBtn_clicked()
{
    ui->controlStackedWidget->setCurrentWidget(ui->controlCollectionOverPage);
}

void uiMine::on_deleteDataOkBtn_clicked()
{
    //删除采集到的数据
    collectionList.clear();
    ui->controlStackedWidget->setCurrentWidget(ui->controlMainPage);
...
void uiMine::on_deleteDataBackBtn_clicked()
{
    ui->controlStackedWidget->setCurrentWidget(ui->controlCollectionOverPage);
}

void uiMine::on_voiceMinusBtn_clicked()
{
    //    g2hDownTaskFileSlot(true, "/storage/taskinfo/d0-s0-empty.tar.gz");
}

void uiMine::on_voicePlusBtn_clicked()
{
    //    g2hDownTaskFileSlot(true, "/storage/taskinfo/s0-d0-full.tar.gz");
}

void uiMine::on_alarmManOkBtn_clicked()
{
...
/*****************zmq***************/
void uiMine::slotTopicData(const void* fbuf,int id)
{
    switch (id) {
    case Topic_ID::TOPIC_TCP_GNSS_GPS_INFO :
    {
        const GpsImuInfo *f = static_cast<const GpsImuInfo*>(fbuf);
        qDebug()<<id<<f->gps_speed()<<"11111111111111111";
    }
        break;
    case Topic_ID::TOPIC_TCP_GNSS_GPS_STATUS :
    {
        const GpsStatus *f = static_cast<const GpsStatus*>(fbuf);
        qDebug()<<id<<f->gps_gps_fault_flag()<<"222222222222222222222";

    }
        break;
    case Topic_ID::TOPIC_IPC_MESSAGE_TEST_TOPIC :
    {
        const Msg8105 *f = static_cast<const Msg8105*>(fbuf);
          qDebug()<<id<<f->time()<<f->result()<<"333333333333333333333333333";
    }
        break;
    default:
        break;
