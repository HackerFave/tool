...
#include "ui_uicollector.h"
#include "HmiFunction/log.h"

uiCollector::uiCollector(QWidget *parent)
    : QWidget(parent)
    , ui(new Ui::uiCollector)
{
    ui->setupUi(this);
...
//    ui->centralStackedWidget->widget(3)->setLayout(workChoiceGrid);
    ...
    abnormalChoice = new AbnormalCollector;
...
void uiCollector::on_stateSetBtn_clicked()
{
    ui->controlStackedWidget->setCurrentWidget(ui->controlStateSetPage);//去状态设置页面
}

void uiCollector::on_loginOutBtn_clicked()
{
...
void uiCollector::on_otherSetBtn_clicked()
{
    ui->controlStackedWidget->setCurrentWidget(ui->controlOtherSetPage);//去往其他设置页面
}

void uiCollector::on_loginOutBackBtn_clicked()
{
...
void uiCollector::on_loginOutOkBtn_clicked()
{
    ui->controlStackedWidget->setCurrentWidget(ui->controlMainPage);//返回首界面
    ui->centralStackedWidget->setCurrentWidget(ui->centralMapViewPage);//返回主显示页面
}

void uiCollector::on_outWorkBackBtn_clicked()
{
...
void uiCollector::on_stateSetBackBtn_clicked()
{
    ui->controlStackedWidget->setCurrentWidget(ui->controlMainPage);//返回首界面
}

//状态设置返回按下
void uiCollector::on_workSetBackBtn_clicked()
{
    ui->controlStackedWidget->setCurrentWidget(ui->controlDiggerWorkPage);//返回首界面
}

void uiCollector::on_faultBackBtn_clicked()
{
    ui->controlStackedWidget->setCurrentWidget(ui->controlStateSetPage);//返回故障页面
    ui->centralStackedWidget->setCurrentWidget(ui->centralMapViewPage);//返回主显示页面
}

void uiCollector::on_faultOkBtn_clicked()
{
    //获取错误号并发送
...
//载货状态返回
void uiCollector::on_loadStateBackBtn_clicked()
{
    ui->centralStackedWidget->setCurrentWidget(ui->centralWorkStatePage);
...
//加油返回
void uiCollector::on_addOilBackBtn_clicked()
{
    ui->centralStackedWidget->setCurrentWidget(ui->centralWorkStatePage);
    ui->controlStackedWidget->setCurrentWidget(ui->controlUnLoadWorkSetPage);//返回状态设置页面
}
...
//加油确定
void uiCollector::on_addOilOkBtn_clicked()
{
    //发送加油信号
    ui->controlStackedWidget->setCurrentWidget(ui->controlMainPage);//返回主界面
}

void uiCollector::on_outWorkOkBtn_clicked()
{
    //发送退出排队信号
...
void uiCollector::on_faultBtn_clicked()
{
...
//就绪申请页面返回按键按下
void uiCollector::on_readyBackBtn_clicked()
{
    ui->controlStackedWidget->setCurrentWidget(ui->controlStateSetPage);//返回首界面
}

//跳转至延时页面
void uiCollector::on_delayBtn_clicked()
{
...
//延时10分钟按键按下
void uiCollector::on_delay10Btn_clicked()
{
//发送延时10分钟信号
...
//延时30分钟按键按下
void uiCollector::on_delay30Btn_clicked()
{
//发送延时30分钟信号
    if(delayChoice->buttonGroup->checkedButton())
    {
...
//延时60分钟按键按下
void uiCollector::on_delay60Btn_clicked()
{
//发送延时60分钟信号
    if(delayChoice->buttonGroup->checkedButton())
    {
...
//延时页面返回
void uiCollector::on_delayBackBtn_clicked()
{
    ui->controlStackedWidget->setCurrentWidget(ui->controlStateSetPage);
    ui->centralStackedWidget->setCurrentWidget(ui->centralMapViewPage);
}

void uiCollector::on_voiceAdjustBtn_clicked()
{
    ui->controlStackedWidget->setCurrentWidget(ui->controlVoicePage);
}

void uiCollector::on_brightAdjustBtn_clicked()
{
    commonUIFunction::instance()->on_brightAdjustBtn_clicked(ui->controlStackedWidget,
                                                             ui->controlBrightPage);
}

void uiCollector::on_editionBtn_clicked()
{
    commonUIFunction::instance()->h2gSendHmiVerCMD();
    ui->controlStackedWidget->setCurrentWidget(ui->controlEditionPage);
}

void uiCollector::on_otherSetBackBtn_clicked()
{
    ui->controlStackedWidget->setCurrentWidget(ui->controlMainPage);//返回首界面
}

void uiCollector::on_brightMinusBtn_clicked()
{
//亮度调低
}

void uiCollector::on_brightPlusBtn_clicked()
{
//亮度调高
}

void uiCollector::on_brightBackBtn_clicked()
{
    ui->controlStackedWidget->setCurrentWidget(ui->controlOtherSetPage);
}

void uiCollector::on_editionBackBtn_clicked()
{
    ui->controlStackedWidget->setCurrentWidget(ui->controlOtherSetPage);
}

void uiCollector::on_voiceBackBtn_clicked()
{
    ui->controlStackedWidget->setCurrentWidget(ui->controlOtherSetPage);
}

void uiCollector::on_emptyOkBtn_clicked()
{
    //发送设置空载信号
...
void uiCollector::on_emptyBackBtn_clicked()
{
    ui->controlStackedWidget->setCurrentWidget(ui->controlLoadStatePage);
}

void uiCollector::on_halfOkBtn_clicked()
{
    //发送设置半载信号
...
void uiCollector::on_halfBackBtn_clicked()
{
    ui->controlStackedWidget->setCurrentWidget(ui->controlLoadStatePage);
}

void uiCollector::on_fullOkBtn_clicked()
{
    //发送设置满载信号
...
void uiCollector::on_fullBackBtn_clicked()
{
    ui->controlStackedWidget->setCurrentWidget(ui->controlLoadStatePage);
}

void uiCollector::g2hPosSlot(struct pubVehiclePosST info)  //网关发送hmi位置信息
{
    PosInfo* pos = PosInfo::getHandle();
    pos->setPosInfo(info);
}

void uiCollector::g2hReqWorkOnStatuSlot(struct pubVehicleWorkStatuST workStatu) //平台发送网关运营状态变更
{
//在信息窗口显示运营状态变更情况
    QString str;
...
void uiCollector::g2hResWorkOnStatuSlot(struct pubVehicleResWorkStatuST cloudReq) //收到平台运营状态变更确认
{
    commonUIFunction::instance()->g2hResWorkOnStatuSlot(ui->messageBrowser,cloudReq);
}

void uiCollector::g2hReqWorkOptSlot(struct pubVehicleReqWorkOptST reqWork) //网关请求现场作业
{
    QString str;
    str.append(QDateTime::currentDateTime().toString("hh:mm:ss :"));
...
void uiCollector::g2hReqWorkRealOptSlot(struct pubVehicleReqWorkOptST reqWork) //网关请求现场作业
{
    commonUIFunction::instance()->g2hReqWorkRealOptSlot(ui->messageBrowser,reqWork);
    mineInfoTimerSlot();
}

void uiCollector::g2hResWorkOptSlot(struct pubVehicleResWorkOptST opt) //辅助车辆（网关已收到矿卡现场作业），矿卡（接收到辅助车辆相应作业信息）
{
    workStateInfo = opt;
}

void uiCollector::g2hCollWarnningSlot(struct pubVehicleCollWarnningST otherVehicle) //预警-碰撞检测
{
//预警显示
...
void uiCollector::g2hDownMapFileSlot(int flag, QString path) //接收网关下载地图命令
{
...
void uiCollector::g2hVersionSlot(struct versionST versiondataSt) //版本号
{
    commonUIFunction::instance()->g2hVersionSlot(ui->editionTable,versiondataSt,&versiondata);
}

void uiCollector::g2hDownTaskFileSlot(bool flag, QString path, QStringList taskList) //下载文件
{
...
void uiCollector::g2hStartUpSlot(struct startUpST) //网关发送开机命令
{
//信息窗口显示，网关开机
    QString str;
...
void uiCollector::g2hShutDownAckSlot() //网关关机应答
{
//信息窗口显示，网管关机
    QString str;
...
void uiCollector::g2hDevNameSlot(struct pubVehicleDevNameST imeiName) //网关检权获取名称后发送给hmi
{
...
void uiCollector::g2hMatAckSlot(short mater) //物料变更
{
//显示物料变更为何物
    QString str;
...
void uiCollector::g2hReqTaskSlot(struct pubVehicleReqTaskST reqTask) //请求任务
{
    commonUIFunction::instance()->g2hReqTaskSlot(ui->messageBrowser,reqTask);

}

void uiCollector::g2hResTaskSlot(struct pubVehicleResTaskST resTask) //接收平台响应任务
{
     commonUIFunction::instance()->g2hResTaskSlot(ui->messageBrowser,ui->titleDestLabel,resTask,taskInfo);
}

void uiCollector::g2hReqRoadSlot(struct pubVehicleReqRoadST reqRoad) //申请路权
{
//信息窗口显示申请路权
    QString str;
...
void uiCollector::g2hResRoadSlot(struct pubVehicleResRoadST resRoad) //平台回复路权申请
{
//信息窗口显示收到路权申请
    QString str;
...
void uiCollector::g2hFreeRoadSlot(struct pubVehicleReqRoadST freeRoad) //平台下发释放路权
{
//信息窗口显示收到平台下发释放路权
    QString str;
...
void uiCollector::g2hTemporaryTaskSlot(char type) //平台下发临时调度作业
{
    commonUIFunction::instance()->g2hTemporaryTaskSlot(ui->messageBrowser,type);
}

void uiCollector::g2hMasterWarnningSlot(struct pubVehicleMasterWarnningST materWarning) //主控发送预警
{
    PosInfo* pos = PosInfo::getHandle();
...
void uiCollector::g2hGatewayOffline(bool status)
{
    commonUIFunction::instance()->g2hGatewayOffline(ui->messageBrowser,status);
}

void uiCollector::insertWarnningSlots(int num)
{
    try
    {
        QString str;
...
void uiCollector::removeWarnningSlots(int num)
{
    try
    {
        QString str;
...
void uiCollector::titleTimerSlot()//界面头信息更新显示
{
    ...
    if((curPos.lat > 0.0000001) && (curPos.lat < 360) && (curPos.lng > 0.0000001) && (curPos.lng < 360))
    {
        if(!map->scene->items().contains(map->selfVehicle))
        {
            if(map->selfVehicle == NULL)
            {
                ...
                if(width<10)width=10;
                if(length<10)length=10;
                map->setSelfVehicle(COLLECTOR, QString::fromUtf8(localImeiName.localname), QSize(width, length));
            }
...
void uiCollector::messageEndTimerSlot()
{
    ui->messageBrowser->moveCursor(QTextCursor::Start);
}

//消息往上滑动
void uiCollector::on_messageUpBtn_clicked()
{
...
//信息往下滑动
void uiCollector::on_messageDownBtn_clicked()
{
    ui->messageBrowser->moveCursor(QTextCursor::Down);
    messageEndTimer->start(2000);
}

void uiCollector::blinkTimerSlot()
{
    if(blinkFlag)
    {
        blinkFlag = false;
...
void uiCollector::collectionTimerSlot()
{
    PosInfo* pos = PosInfo::getHandle();
#if TEST
    struct latlngheading *curPos = new struct latlngheading;

    struct pubVehiclePosST posSt;
    if(testnum < testlist.size())
    {
        posSt.gLat = testlist.at(testnum)->lat;
        posSt.gLng = testlist.at(testnum)->lng;
        posSt.gHead = testlist.at(testnum)->heading;
        pos->setPosInfo(posSt);

        curPos = testlist.at(testnum);
        collectionList.append(curPos);

        testnum += 1;
    }
#else
    struct collectlatlngheading* collPos = new struct collectlatlngheading;
    pos->getCurPos(collPos);
    collPos->flag = false;
    collPos->height -= ParaParsing::instance()->paraSt.collector.gpsHeightofVehicle;
    map->addCollectionPoint(collPos);
#endif
...
void uiCollector::mineInfoTimerSlot()
{
...
void uiCollector::on_rightBtn_clicked()
{
    ui->suspendRestartBtn->setChecked(false);
    map->removeCollection();
    areaside = 0x02;
...
void uiCollector::on_suspendRestartBtn_clicked()
{
    PosInfo* pos = PosInfo::getHandle();
    ...
    if(ui->suspendRestartBtn->isChecked())
    {
        ...
        collData->flag = true;
        collData->height -= ParaParsing::instance()->paraSt.collector.gpsHeightofVehicle;
...
void uiCollector::on_stopBtn_clicked()
{
    ui->controlStackedWidget->setCurrentWidget(ui->controlCollectionOverPage);
}

void uiCollector::on_collectionOverBackBtn_clicked()
{
    ui->controlStackedWidget->setCurrentWidget(ui->controlCollectionStatePage);
}

void uiCollector::on_saveBtn_clicked()
{
    //生成边界文件lpx
    collectionTimer->stop();
    ...
    boundaryDataFixFun(&map->realTimeItem->list, sidefixvalue, parahandle->paraSt.collector.boundaryFixValue);
    QApplication::processEvents();
...
void uiCollector::on_deleteBtn_clicked()
{
    collectionTimer->stop();
    map->removeCollection();
    //map->datathread->clearRtcCollectionList();
...
void uiCollector::on_updateDataOkBtn_clicked()
{
    //提交边界文件
    fileUpST roadFile;
...
void uiCollector::on_updateDataBackBtn_clicked()
{
    ui->controlStackedWidget->setCurrentWidget(ui->controlCollectionOverPage);
}

void uiCollector::on_deleteDataOkBtn_clicked()
{
    //删除采集到的数据
    map->datathread->clearRtcCollectionList();
    map->removeCollection();
...
void uiCollector::on_deleteDataBackBtn_clicked()
{
    ui->controlStackedWidget->setCurrentWidget(ui->controlCollectionOverPage);
}

void uiCollector::on_abnormalOkBtn_clicked()
{
...
void uiCollector::on_abnormalBtn_clicked()
{
...
void uiCollector::on_abnormalBackBtn_clicked()
{
    ui->controlStackedWidget->setCurrentWidget(ui->controlUnLoadWorkSetPage);
    ui->centralStackedWidget->setCurrentWidget(ui->centralMapViewPage);
}

void uiCollector::on_unLoadRunWorkBtn_clicked()
{
    WorkListDeal* worklist = WorkListDeal::getHandle();
    struct pubVehicleDevNameStateST* firstNode = new struct pubVehicleDevNameStateST;
    bool result = false;
    ...
    if(!ui->unLoadRunWorkBtn->isChecked())//false
    {
        //发送允许卸料
...
void uiCollector::on_unLoadOverOutBtn_clicked()
{
    WorkListDeal* worklist = WorkListDeal::getHandle();
    struct pubVehicleDevNameStateST* firstNode = new struct pubVehicleDevNameStateST;
...
void uiCollector::on_voiceMinusBtn_clicked()
{
    //音量调低
...
void uiCollector::on_voicePlusBtn_clicked()
{
    //音量调高
...
void uiCollector::on_useCurBoundaryBtn_clicked()
{
    StopListDeal* handle = StopListDeal::getHandle();
    ParaParsing* parahandle = ParaParsing::instance();
...
void uiCollector::on_diggerWorkBackBtn_clicked()
{
    ui->controlStackedWidget->setCurrentWidget(ui->controlMainPage);
}

void uiCollector::on_collectionBtn_clicked()
{
    ui->controlStackedWidget->setCurrentWidget(ui->controlCollectionPage);
}

void uiCollector::on_updateOldDataBackBtn_clicked()
{
    ui->controlStackedWidget->setCurrentWidget(ui->controlCollectionPage);
}

void uiCollector::on_updateOldDataOkBtn_clicked()
{
    on_updateDataOkBtn_clicked();
}

void uiCollector::on_collectorBtn_clicked()
{
    ui->controlStackedWidget->setCurrentWidget(ui->controlCollectionPage);
}
