...
client_ssl::client_ssl(const std::string &ca,const std::string &client_cert, const std::string & privateKey,const std::string & ip,const std::string & port):
    _ca(ca),
...
void client_ssl::init()
{
    ...
    {
        //        _testData.num = 1;
        //        strcpy(_testData.str,"我在测试数据");
        //        _testData.checked = true;
        //        if(writeData(&_testData,sizeof (struct testData))){
        //            std::cout<<"[发送消息成功：]"<<_testData.str<<std::endl;
        //        }
        //        char buffer[MAXBUF + 1];
        //        if(readData(buffer,MAXBUF)){
        //            std::cout<<"[接收消息：]"<<buffer<<std::endl;
        //        }

        //        pthread_t threads[2];
        //        int indexes[2];// 用数组来保存i的值
        //        int rc;
        //        int i;
        //        struct mypara pstru1;
        //        struct mypara pstru2;
        //        pstru1.para1 = buffer;
        //        pstru2.para2 = MAXBUF;

        //        pstru2.para1 = &_testData;
        //        pstru2.para2 = sizeof (struct testData);

        //        rc = pthread_create(&threads[0], NULL,threadRead, (&pstru1));
        //        if (rc){
        //            cout << "Error:无法创建线程," << rc << endl;
        //            exit(-1);
        //        }
        //        rc = pthread_create(&threads[1], NULL, threadWrite,(&pstru2));
        //        if (rc){
        //            cout << "Error:无法创建线程," << rc << endl;
        //            exit(-1);
        //        }
...
void  client_ssl::threadRead(int para)
{
    ...
    while(1){
        ...
        if( p_client_ssl->readData(buffer,MAXBUF)){
            std::cout<<"[接收消息：]"<<buffer<<"num"<<i++<<std::endl;
        }
        sleep(3);
    }
}
