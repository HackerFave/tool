...
int main(void)
{
	...
	port_printf("i2c common test begin\n");

	//---- debug---------------------
	do
	{
		//---- 1. 注册I2C 通信外设:  SE0 ----
		ret_code = api_register(PERIPHERAL_I2C, I2C_PERIPHERAL_SE0);
		if(ret_code != SE_SUCCESS)
		{
			port_printf("\nfailed to api_register!");
			break;
		}
		port_printf("\nsuccess to register i2c se0!");

		//---- 2. 选择I2C 通信外设:  SE0 ----
		ret_code =api_select(PERIPHERAL_I2C, I2C_PERIPHERAL_SE0);
		if(ret_code != SE_SUCCESS)
		{
			port_printf("\nfailed to api_select!");
			break;
		}
		port_printf("\nsuccess to select i2c se0!");
	...
	//循环切换发送命令测试
	#if defined (HED_I2C_COMM_SWITCH) 
	...
	while (1)
	{
		HAL_GPIO_TogglePin(PORT_I2C_LED_CTRL_PORT,PORT_I2C_LED_CTRL_IO_PIN);
		HAL_Delay(1000);	
	}
}
