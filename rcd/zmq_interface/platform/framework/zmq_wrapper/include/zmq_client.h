...
public:
   ...
   zmsg *
   send (std::string service, zmsg *&request_p, int timeout = 2500)
   {
       assert (request_p);
       ...
       while (retries_left && !s_interrupted) {
           ...
           while (!s_interrupted) {
               ...
               if (items [0].revents & ZMQ_POLLIN) {
                   ...
                   std::string str_header((char *)header.data(), header.size());
                   assert (str_header.compare(MDPC_CLIENT) == 0);

                   std::vector<uint8_t> reply_service = recv_msg->pop_front();
                   std::string str_reply((char *)reply_service.data(), reply_service.size());
