...
// 函数方法生成密钥对
void generateRSAKey(std::string private_key,std::string public_key)
{
    // 公私密钥对
    size_t pri_len;
    size_t pub_len;
...
//    PEM_write_bio_RSA_PUBKEY(pub, keypair);
    ...
    public_key = pub_key;
    private_key = pri_key;
    std::cout <<"1111111111111111111111"<<pri_key<<std::endl;
    std::cout <<"2222222222222222222222222"<<pub_key<<std::endl;
    write_string_to_file_append("./scm/config/public_key.pem",public_key);
    write_string_to_file_append("./scm/config/private_key.pem",private_key);
    // 内存释放
    RSA_free(keypair);
    BIO_free_all(pub);
    BIO_free_all(pri);
...
bool MakeCsrSSL(const  char * keyFilePath, const  char *email,
                const  char *name, const  char *country, const  char *saveCsrFilePath)
{
    int             ret = 0;
    RSA             *r = NULL;
    BIGNUM          *bne = NULL;
...
signSdk::signSdk()
{
    std::string pri = "";
    std::string pub = "";
    generateRSAKey(pri,pub);

    std::string keyFilePath = "./scm/config/private_key.pem";
    std::string email = "111";
    std::string name = "222";
    std::string country = "country";
    std::string saveCsrFilePath = "./scm/config/client.csr";
    MakeCsrSSL( keyFilePath.c_str(),email.c_str(),name.c_str(),country.c_str(),saveCsrFilePath.c_str());
    //    readFile("./key.txt");
    //    _alg  = SCM_RSA;
    //    init_sdk();
    ...
    //    _client.reset(new client_ssl(ca,client_cert,privateKey,_ip,_port));

    //        _client->init();
    //        _mq_client.reset(new mq_client(ca,client_cert,privateKey,_ip,_port));
}
...
void signSdk::init_sdk() {
    ...
    IW_InitSSLCertChain(/*CACerts*/_certs.c_str());
    //初始化用户证书的证书链，用于校验后续的用户证书的合法新
    IW_InitCACertChain(/*CACerts*/_certs.c_str());
...
void signSdk::get_cert()
{
    ...
    strcpy(csrInfo.subjectInfo.commonName,      _imei.c_str());
...
void signSdk::importexport_cert( unsigned char * client_cert,int length)
{
    ...
    IW_ExportPublicKey(_userId.c_str(),_pin.c_str(),publicKey,&pub_len);
...
int signSdk::LoadConfig(string ConfigPath)
{
    ...
    _imei = tag_setting.get<string>("imei");
    if(_ip==""||_port==""||_userId==""||_pin==""||_imei==""){
        return -1;
    }
    
    return 1;
}
bool signSdk::readFile(std::string path)
{
    string stringLine;
    ifstream infile;
    ...
    while( !infile.eof() ) // To get you all the lines.
    {
        ...
        _certs = _certs+"\n"+stringLine;
        //        strcat(_CACerts,stringLine.c_str());
        ...
        //            }
        //            if ( tempLine[0][0] == 'B')
        //            {   tempName = tempLine[0].substr(0,tempLine[0].length() - 2);
        //                rootName.push_back( tempName );
        //                fout << tempName << endl;
        //            }
        
        //            fout << tempLine[1] << "  "<< tempLine[2] << "  "<< tempLine[3] << endl;
        //            tempLine.clear();
    }
    std::cout<<"****"<<_certs<<"*****************"<<std::endl;
    //    strcpy(_CACerts,_certs.c_str());
    infile.close();
}
